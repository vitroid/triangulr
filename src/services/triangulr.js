...
Triangulr.prototype.setContainer = function (container) {
  ...
  if (!container) {
    throw new Error(
      'Triangulr container "' + containerId + '" does not exists.'
    );
  }
...
Triangulr.prototype.setCanvas = function (width, height, isLandscape) {
  ...
  this.triangleHeight = Math.sqrt(
    Math.pow(this.triangleWidth, 2) - Math.pow(this.triangleWidth / 2, 2)
  );
...
Triangulr.prototype.endSelection = function () {
  ...
  this.indexesFromCoordinates(offsetX, offsetY, width, height).map((index) => {
...
Triangulr.prototype.updateSelectionDrag = function (position) {
  ...
  if (this.isLandscape) {
    ...
    if (
      newX >= 0 &&
      newX + coor.width <= this.lineLength &&
      newY >= 0 &&
      newY + coor.height <= this.mapHeight
    ) {
      coor.dragX = dragX;
      coor.dragY = dragY;
      this.selection.selectArea.style.transform = `translate(${
        (coor.moveX + dragX) * this.blockWidth
      }px,${(coor.moveY + dragY) * this.triangleHeight}px)`;
    }
  } else {
    ...
    if (
      ...
      coor.dragY = dragY;
      this.selection.selectArea.style.transform = `translate(${
        (coor.moveX + dragX) * this.triangleHeight
      }px,${(coor.moveY + dragY) * this.blockWidth}px)`;
    }
  }
};
...
Triangulr.prototype.applySelection = function () {
  if (
    !this.selection ||
    !this.selection.coordinates ||
    (!this.selection.coordinates.moveX &&
      !this.selection.coordinates.moveY &&
...
Triangulr.prototype.fillTriangle = function (pos, color) {
  ...
  this.exportData[pos].color =
    color === undefined ? null : color || this.exportData[pos].color;
  this.svgTag.childNodes[pos].setAttribute(
    "fill",
    this.exportData[pos].color || this.BLANK_COLOR
  );
};
...
Triangulr.prototype.generateSVG = function (isClean) {
  ...
  if (this.isLandscape) {
    ...
    svgTag.setAttribute(
      "viewBox",
      "0 0 " +
        this.mapWidth * this.triangleWidth +
        " " +
        this.mapHeight * this.triangleHeight
    );
  } else {
    ...
    svgTag.setAttribute(
      "viewBox",
      "0 0 " +
        this.mapWidth * this.triangleHeight +
        " " +
        this.mapHeight * this.triangleWidth
    );
  }
...
Triangulr.prototype.import = function (data) {
  this.setCanvas(data.mapWidth, data.mapHeight, data.isLandscape);
  ...
  for (var i = 0; i < this.svgTag.childNodes.length; i++) {
    this.svgTag.childNodes[i].setAttribute(
      "fill",
      this.exportData[i].color || this.BLANK_COLOR
    );
  }
};
...
Triangulr.prototype.export = function () {
  return {
    ...
    mapWidth: this.mapWidth,
    mapHeight: this.mapHeight,
    mapData: this.exportData.map(function (e) {
      return e.color || null;
    }),
    palette: this.palette,
  };
...
Triangulr.prototype.loadWorkspaceFromFile = function (data) {
  ...
  if (data[0] === "{") {
    ...
    svg.querySelectorAll("path").forEach((path) => {
      config.mapData[parseInt(path.getAttribute("rel"), 10)] =
        path.getAttribute("fill") || this.DEFAULT_FILL_COLOR;
    });
  }
...
Triangulr.prototype.undo = function () {
  let fill,
    backAction = this.backStack.popLastAction();
